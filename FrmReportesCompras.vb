...
Imports System.IO
Public Class FrmReportesCompras
    ...
    Private Sub FrmReportesCompras_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            conection.Open()
            Dim cmd As New MySqlCommand("Select Idproducto, Cantidad, Fecha FROM compras", conection)
            Dim drd As MySqlDataReader = cmd.ExecuteReader(CommandBehavior.SingleResult)
            Dim bs As New BindingSource
            bs.DataSource = drd
            DgCompra.DataSource = bs
            drd.Close()
            conection.Close()
    ...
    Private Sub BtnConsultar_Click(sender As Object, e As EventArgs) Handles BtnConsultar.Click
        Dim conection As New MySqlConnection("Server=127.0.0.1;User=root;Password=18112004;Port=3306;database=existenciaproductos")
        Dim table As New DataTable()
        Dim adaptador As New MySqlDataAdapter("Select Idproducto, Cantidad, Fecha from compras where Fecha like '_%" & CBFecha.Text & "%__'", conection)
        adaptador.Fill(table)
        DgCompra.DataSource = table
    End Sub
    ...
    Private Sub BtnImprimir_Click(sender As Object, e As EventArgs) Handles BtnImprimir.Click
        Try
            ...
            Dim filename As String = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\Lista de Compra de productos.pdf"
    ...
    Public Sub ExportarDatosPDF(ByVal document As Document)
        Dim datatable As New PdfPTable(DgCompra.ColumnCount)
        datatable.DefaultCell.Padding = 3
        Dim headerwidths As Single() = GetColumnasSize(DgCompra)
        ...
        Dim encabezado As New Paragraph("Reporte de Compras", New Font(Font.Name = "Tahoma", 20, Font.Bold))
        'SE CREA EL TEXTO ABAJO DEL ENCABEZADO
        Dim texto As New Phrase("Reporte de Compras realizadas en el mes: " + CBFecha.Text(), New Font(Font.Name = "Tahoma", 14, Font.Bold))
        For i As Integer = 0 To DgCompra.ColumnCount - 1
            datatable.AddCell(DgCompra.Columns(i).HeaderText)
        Next
        datatable.HeaderRows = 1
        datatable.DefaultCell.BorderWidth = 1
        'SE GENERA LAS COLUMNAS DEL DATAGRID
        For i As Integer = 0 To DgCompra.RowCount - 1
            For j As Integer = 0 To DgCompra.ColumnCount - 1
                datatable.AddCell(DgCompra(j, i).Value.ToString())
            Next
